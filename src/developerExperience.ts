...
export interface DeveloperInsight {
    type: 'performance' | 'security' | 'maintainability' | 'learning' | 'productivity';
    title: string;
    description: string;
...
export class DeveloperExperience {
    ...
    private insights: DeveloperInsight[] = [];
    private keystrokePattern: number[] = [];
    private lastActivity: Date = new Date();
    private productivityMetrics = {
        linesWritten: 0,
        errorsFixed: 0,
        testsWritten: 0,
    ...
    private setupProductivityTracking() {
    ...
    private analyzeKeystrokeRhythm(): { isConsistent: boolean; averageInterval: number } {
    ...
    private updateFocusScore(rhythm: { isConsistent: boolean; averageInterval: number }) {
    ...
    private handleFileSwitch(fileName: string) {
    ...
    private trackErrorResolution(event: vscode.DiagnosticChangeEvent) {
    ...
    public addInsight(insight: DeveloperInsight) {
        this.insights.push({
            ...insight,
    ...
    private showInsightNotification(insight: DeveloperInsight) {
    ...
    private executeInsightAction(insight: DeveloperInsight) {
    ...
    public async analyzeCodeQuality(document: vscode.TextDocument): Promise<DeveloperInsight[]> {
        ...
        const qualityInsights: DeveloperInsight[] = [];

        if (analysis?.predictions) {
            analysis.predictions.forEach((prediction: string) => {
    ...
    public async suggestRefactoring(selection: vscode.Selection, document: vscode.TextDocument): Promise<void> {
    ...
    public getInsights(): DeveloperInsight[] {
    ...
    public async predictNextAction(): Promise<string> {
    ...
    public async generateContextualHelp(): Promise<void> {
